...
function Manager(opts)
{
	...
	this.saving = opts.save_data || false;
	this.loading = opts.load_data || false;
...
Manager.prototype.initialize = function() {
	...
	if (this.loading) {
		...
		this.onStorageLoaded({lowest: 60, seconds: {}, clicks: 0});
	}
};
...
Manager.prototype.listen = function() {
	this.storage.once('loaded', this.onStorageLoaded.bind(this));
	this.core.once('loaded', this.onCoreLoaded.bind(this));
...
Manager.prototype.onStorageLoaded = function(payload) {
...
Manager.prototype.onCoreLoaded = function() {
...
Manager.prototype.onClick = function(click) {
	...
	this.onClickData(click, (this.latestClicks.length == 0 ? 'first' : 'last'));
...
Manager.prototype.onClickData = function(data, type) {
	var day = moment(data.tickTime, "YYYY-MM-DD-HH-mm-ss Z").format('YYYY-MM-DD');
...
Manager.prototype.onTick = function(tick) {
	...
	var text = '[' + moment(tick.tickTime, "YYYY-MM-DD-HH-mm-ss Z").format('YYYY-MM-DD HH:mm:ss.SSS') + '] ' + tick.second + '.00 seconds ';
...
Manager.prototype.drawHeader = function() {
	...
	// Header
	this.text +=         colors.bold + 'First Click: ' + colors.reset + moment(this.firstClick.tickTime, "YYYY-MM-DD-HH-mm-ss Z").format('YYYY-MM-DD HH:mm:ss');
	this.text += '    ' + colors.bold + 'Last Click: ' + colors.reset + moment(this.lastClick.tickTime, "YYYY-MM-DD-HH-mm-ss Z").format('YYYY-MM-DD HH:mm:ss');
	this.text += '    ' + colors.bold + 'Days Elapsed: ' + colors.reset + days;
	
	this.addRow();
	
	this.text +=          colors.bold + 'Total Clicks: ' + colors.reset + this.totalClicks + ' (+' + this.lastClick.clicks + ')';
...
Manager.prototype.drawLatest = function() {
	// Latest Clicks
	this.text += colors.bold + 'Latest clicks:' + colors.reset;
	this.addRow();
	...
	for (var i in this.latestClicks) {
		...
		var unitText = '[' + moment(click.tickTime, "YYYY-MM-DD-HH-mm-ss Z").format('YYYY-MM-DD HH:mm:ss.SSS') + '] ' + click.second + '.00 seconds ';
		while (diff > 0) {
			unitText += '#';
			diff--;
		}
